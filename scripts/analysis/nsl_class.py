# Structured NSL Data
class NSL:
    def __init__(self, issue_date, nsl_file_number_only, year=None, company=None, release_date=None, link_to_nsl_file=None, link_to_release_letter=None, comment=None):
    ...
    def __str__(self):
        return f"NSL({self.nsl_file_number_only}, {self._format_datetime(self.issue_date)})"
    
    def export_as_csv_line(self):
        """
        Format: issue date, release date, company, file number, url, comment
        """
        line_data = [self._format_datetime(self.issue_date), self._format_datetime(self.release_date), self.company, self.nsl_file_number_only, self.link_to_nsl_file, self.comment]
